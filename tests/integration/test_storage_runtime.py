...
def _planning_index(include_entry: bool = True) -> PlanningIndexSchema:
    records = ()
    if include_entry:
        records = (
            PlanningIndexEntry(
                planning_id="planning/index/example-1",
                source_records=(
                    PlanningIndexSourceRecord(
                        source_name="source-a",
                        source_record_id="source-a:1",
...
def _embedding_cache(include_entry: bool = True) -> EmbeddingCacheCatalog | None:
...
def test_storage_runtime_integrates_selected_example_package() -> None:
    ...
    assert runtime.package_build.package_manifest.manifest_id == "package:package-001:v1"
...
def test_storage_runtime_reports_missing_planning_and_cache_inputs() -> None:
    runtime = integrate_storage_runtime(
        _package_manifest(),
        planning_index=_planning_index(include_entry=False),
        canonical_store=_canonical_store(),
        feature_cache=_feature_cache(),
