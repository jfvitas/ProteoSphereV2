...
DEFAULT_OUTPUT_MD = (
    REPO_ROOT / "docs" / "reports" / "external_dataset_provenance_audit_preview.md"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export external dataset provenance audit preview."
    )
...
def build_external_dataset_provenance_audit_preview(
    *args: dict[str, Any],
) -> dict[str, Any]:
    return build_external_dataset_audits(*args)["provenance"]
...
def main() -> None:
    ...
    payload = payloads["provenance"]
    write_json(args.output_json, payload)
    write_text(
        args.output_md,
        render_markdown("External Dataset Provenance Audit Preview", payload),
    )
