...
DEFAULT_OUTPUT_JSON = REPO_ROOT / "artifacts" / "status" / "package_readiness_preview.json"
DEFAULT_OUTPUT_MD = REPO_ROOT / "docs" / "reports" / "package_readiness_preview.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export package readiness preview.")
...
def main() -> None:
    args = parse_args()
    payload = build_package_readiness_preview(
        ...
        read_json(args.split_engine_input),
        read_json(args.split_dry_run_validation),
        read_json(args.split_fold_export_gate),
        read_json(args.split_post_staging_gate_check),
    )
    write_json(args.output_json, payload)
    write_text(args.output_md, render_markdown("Package Readiness Preview", payload))
