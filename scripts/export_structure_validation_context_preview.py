...
DEFAULT_OUTPUT_MD = REPO_ROOT / "docs" / "reports" / "structure_validation_context_preview.md"


def build_structure_validation_context_preview(
    ...
    rows = [
        {
            ...
            "source_api_statuses": row.get("source_api_statuses") or {},
        }
    ...
    return {
        ...
        "schema_id": "proteosphere-structure-validation-context-preview-2026-04-03",
        ...
        "truth_boundary": {
            "summary": "Validation context is derived from structured entry harvests only.",
            "report_only": True,
            "governing": False,
        },
    }
...
def render_markdown(payload: dict[str, Any]) -> str:
    return (
        "# Structure Validation Context Preview\n\n"
        + "\n".join(
            (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build structure validation context preview.")
    parser.add_argument(
...
def main() -> None:
    args = parse_args()
    payload = build_structure_validation_context_preview(read_json(args.structure_entry_context))
