...
DEFAULT_OUTPUT_MD = (
    REPO_ROOT / "docs" / "reports" / "external_dataset_binding_audit_preview.md"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export external dataset binding audit preview.")
...
def build_external_dataset_binding_audit_preview(
    *args: dict[str, Any],
) -> dict[str, Any]:
    return build_external_dataset_audits(*args)["binding"]
...
def main() -> None:
    ...
    write_text(
        args.output_md,
        render_markdown("External Dataset Binding Audit Preview", payload),
    )
