...
from scripts.kinetics_enzyme_support_common import (
    ...
    _read_json,
    build_kinetics_enzyme_support_validation,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate the kinetics/enzyme accession-level support preview."
    )
    parser.add_argument("--kinetics-support-preview", type=Path, default=DEFAULT_OUTPUT_JSON)
    ...
    parser.add_argument(
        "--pdb-chain-enzyme",
        type=Path,
        default=DEFAULT_PDB_CHAIN_ENZYME,
    )
    parser.add_argument("--operator-dashboard", type=Path, default=DEFAULT_OPERATOR_DASHBOARD)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_VALIDATION_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_VALIDATION_MD)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    payload = build_kinetics_enzyme_support_validation(
