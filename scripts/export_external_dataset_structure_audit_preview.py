...
DEFAULT_OUTPUT_MD = (
    REPO_ROOT / "docs" / "reports" / "external_dataset_structure_audit_preview.md"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export external dataset structure audit preview.")
...
def build_external_dataset_structure_audit_preview(
    *args: dict[str, Any],
) -> dict[str, Any]:
    return build_external_dataset_audits(*args)["structure"]
...
def main() -> None:
    ...
    write_text(
        args.output_md,
        render_markdown("External Dataset Structure Audit Preview", payload),
    )
