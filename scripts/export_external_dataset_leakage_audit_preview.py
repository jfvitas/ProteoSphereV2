...
DEFAULT_OUTPUT_MD = (
    REPO_ROOT / "docs" / "reports" / "external_dataset_leakage_audit_preview.md"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export external dataset leakage audit preview.")
...
def build_external_dataset_leakage_audit_preview(
    *args: dict[str, Any],
) -> dict[str, Any]:
    return build_external_dataset_audits(*args)["leakage"]
...
def main() -> None:
    ...
    payload = payloads["leakage"]
    write_json(args.output_json, payload)
    write_text(
        args.output_md,
        render_markdown("External Dataset Leakage Audit Preview", payload),
    )
