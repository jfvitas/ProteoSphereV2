...
def main() -> None:
    ...
    write_json(args.output_json, payloads["top_level"])
    write_text(
        args.output_md,
        render_markdown("External Dataset Assessment Preview", payloads["top_level"]),
    )
    write_json(DEFAULT_LEAKAGE_OUTPUT_JSON, payloads["leakage"])
    write_json(DEFAULT_MODALITY_OUTPUT_JSON, payloads["modality"])
    write_json(DEFAULT_BINDING_OUTPUT_JSON, payloads["binding"])
    write_json(DEFAULT_STRUCTURE_OUTPUT_JSON, payloads["structure"])
    write_json(DEFAULT_PROVENANCE_OUTPUT_JSON, payloads["provenance"])
    print(json.dumps(payloads["top_level"], indent=2))
